...
export function worldSelectionSpacing(): Plugin {
  return {
    ...
    transform(code, id) {
      ...
      // La ligne `threat` n'existe plus après le nettoyage ci-dessus :
      // supprime uniquement ses repositionnements pour éviter que les badges
      // INFINI ÉVOLUTIF / INFINI CAUCHEMAR disparaissent au rendu.
      transformed = transformed.replace(
