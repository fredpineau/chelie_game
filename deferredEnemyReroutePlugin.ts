 ...
 *
 * If the normal reroute cannot find a route from its nearest free start cells,
 * a rare fallback looks slightly farther away. A fallback start is accepted
 * only when the connector from the enemy's real position to that grid point
 * does not cross any plant, so it cannot create a shortcut through the maze.
 */
