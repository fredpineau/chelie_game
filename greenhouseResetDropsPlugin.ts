...
export function greenhouseResetDrops(): Plugin {
  return {
    ...
    transform(code, id) {
      ...
      // wateringGuideMastery() runs before this post plugin and rewrites the
      // original static mastery section. Anchor on that final structure instead
      // of the pre-transform source so the build stays stable.
      ...
      if (!code.includes(anchor)) {
        throw new Error("Greenhouse reset anchor not found after mastery guide transform.");
      }
